...
### [パーツ](/guide/item/)
壁や床など、マップは「[パーツ](/guide/item/)」によって構成されます。

### レイヤー
パーツは「レイヤー」とよばれる層状の領域で管理されています。レイヤーの番号が小さいほど奥に、大きいほど手前に表示されます。

::: tip キャラクタのレイヤー配置
[キャラクタ](/guide/avatar/)はレイヤー2に配置されます。
:::

### マップチップ
パーツのほかに、マップチップを並べてマップをつくることができます。くわしくは [マップチップ](/guide/tiled/) を参照してください。

### 背景
背景を使うと遠景を表現できます。くわしくは、[背景](#背景の設定)を参照してください。

### コネクタ
[コネクタ](/guide/connector/) で異なるマップと接続できます。
...
- マップサイズは面積が3200x3200pxが最大値です。たとえば幅を小さくするとその分高さを大きく取ることができます。
- 最小サイズは幅・高さ共に300pxです。
...
![見下ろしビュー](./images/topview.png)
（このマップの画像はぴぽやさん（[https://pipoya.net/](https://pipoya.net/)）の素材をお借りしてつくりました）

見下ろしビューモードに設定すると、RPGのマップのように上から見下ろしたマップを作成できます

ツールバーの「マップの設定」をクリックします。

![マップの設定](./images/topview2.png)

表示された画面の「見下ろしビューモード」にチェックを入れてOKを押します。

![見下ろしビューモードの設定](./images/topview3.png)

## 背景の設定
![遠景](./images/scroll.gif)

キャラクター移動時のスクロール量を調整できるので、かんたんに遠景を表現できます。背景はレイヤー0に配置されます。
...
背景ボタンを押し、設定したいアニメーションを選択します。「スクロール量」を1より小さくすると、背景のスクロールがキャラクター移動によるスクロールに対して遅くなるので、遠景を表現することができます。

![背景設定ウインドウ](./images/background-window.png)

パーツをレイヤー0に配置した場合もスクロール量が適用されます。パーツを並べることによっても背景を作ることができます。

![レイヤーを0にする](./images/layer-zero.png)
...
作ったマップを探検するには、ツールバーから「このマップを探検する」をクリックします。
